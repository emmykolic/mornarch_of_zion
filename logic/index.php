...
class index extends boiler
{
	...
	public function  defaultb(){
		...
		$about_fields = $this->db->query("SELECT * FROM about_fields ORDER BY aid DESC LIMIT 1");


		// $trending_music = $trending_music->fetch_assoc();
		// $get_blog = $this->db->query("SELECT * FROM blogs ORDER BY bid  OFFSET 0");
		$get_blog = $this->db->query("SELECT * FROM blogs WHERE views > 10 ORDER BY views DESC LIMIT 2");
		...
		$index = date('z') % count($scriptures);

		$daily_verse    = $scriptures[$index]['verse'];
		$bible_passage  = $scriptures[$index]['ref'];

		function word_limiter($text, $limit = 7) {
	...
	{
		$this->page_title = "Privacy Policy — Monarch of Zion";
		$this->set_token();
		include_once 'themes/' . $this->setting->landing_theme . '/header.php';
		include_once 'themes/' . $this->setting->landing_theme . '/privacy_policy.php';
		include_once 'themes/' . $this->setting->landing_theme . '/footer.php';
	}
	...
	{
		$this->page_title = "Terms of Service — Monarch of Zion";
		$this->set_token();
		include_once 'themes/' . $this->setting->landing_theme . '/header.php';
		include_once 'themes/' . $this->setting->landing_theme . '/terms_of_service.php';
		include_once 'themes/' . $this->setting->landing_theme . '/footer.php';
	}
